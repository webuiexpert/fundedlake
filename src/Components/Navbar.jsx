...
const Navbar = () => {
  ...
  return (
    <nav className="bg-dark text-white shadow-md font-lexend sticky top-0 z-[999]">
      <div className="container w-full max-w-6xl h-100 mx-auto px-4 py-4 md:py-6 lg:px-0 lg:py-4 flex items-center justify-between">
        {/* Logo */}
      ...
      {isOpen && (
        <div className="lg:hidden mt-3 space-y-2 text-center flex flex-col md:items-center md:justify-center md:px-0 px-4 pb-6 items-start md:gap-4 gap-5">
          ...
          <a className='hover:text-primary' href="https://fundedlakedashboard.propaccount.com/en/affiliates" target='_blank'>AFFILIATE PROGRAM</a>
          <Link to="/dashboard" className="block bg-sky-500 text-white md:max-w-[260px] px-4 py-2 rounded font-bold">
            DASHBOARD
          </Link>
          <Link to="/get-funded" className="block bg-yellow-400 text-black md:max-w-[260px] px-4 py-2 rounded font-bold">
            GET FUNDED
          </Link>
        </div>
      )}
